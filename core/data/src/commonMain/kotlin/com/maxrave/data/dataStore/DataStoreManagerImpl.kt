...
internal class DataStoreManagerImpl(
    ...
    override val cleanTrackTitles: Flow<String> =
        settingsDataStore.data.map { preferences ->
            preferences[CLEAN_TRACK_TITLES] ?: TRUE
        }

    override suspend fun setCleanTrackTitles(clean: Boolean) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[CLEAN_TRACK_TITLES] = if (clean) TRUE else FALSE
            }
        }
    }
    ...
    override val lyricsOffset: Flow<Long> =
        settingsDataStore.data.map { preferences ->
            preferences[LYRICS_OFFSET] ?: 0L
        }

    override suspend fun setLyricsOffset(offsetMs: Long) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
